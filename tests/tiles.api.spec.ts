...
test('GET /data/tileset-world.json returns TileJSON with fixture metadata', async () => {
  const res = await anonymousAx.get('/data/tileset-world.json')
  expect(res.status).toBe(200)
...
test('GET /styles/basic/style.json returns the normalized style', async () => {
  ...
  expect(style.sources.openmaptiles.url).toContain('/data/tileset-world.json')
})
